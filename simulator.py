...
from pathfinding import find_all_paths
...
class Simulator:
    def __init__(self, result: ParseResult, graph: Graph) -> None:
        ...
        self.drones = [
            Drone(id=i, current_zone=result.start.name)
            for i in range(1, result.nb_drones + 1)
        ]
        paths = find_all_paths(
            result.start, result.end, graph, result.nb_drones
        )
        if not paths or not paths[0]:
            raise ValueError(
                f"No path found from '{result.start.name}' to "
                f"'{result.end.name}' — check if goal is reachable "
                f"(blocked zones?)"
            )
    ...
    def run(self) -> None:
        ...
        while not all(drone.arrived for drone in self.drones):
            ...
            for dr in self.drones:
                if not dr.arrived:
                    zone_occupancy[dr.current_zone] = (
                        zone_occupancy.get(dr.current_zone, 0) + 1
                    )
            ...
            for dr in self.drones:
                if (dr.arrived or dr.in_transit
                        or dr.id in moved_this_turn):
                    continue
                ...
                max_conn = self.graph.hubs[
                    dr.current_zone
                ].links_capacity.get(nxt, 1)

                if current_on_conn >= max_conn:
                    continue

                if (dr.current_zone in zone_occupancy
                        and zone_occupancy[dr.current_zone] > 0):
